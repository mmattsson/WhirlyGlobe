...
float4 basicPos(const thread VertexTriA &vert,
                const constant Uniforms &uniforms,
                const constant UniformDrawStateA &uniDrawState)
{
    const float3 vertPos = (uniDrawState.singleMat * float4(vert.position,1.0)).xyz;
    return uniDrawState.clipCoords ? float4(vertPos,1.0) :
        uniforms.pMatrix * (uniforms.mvMatrix * float4(vert.position,1.0) +
                            uniforms.mvMatrixDiff * float4(vert.position,1.0));
}
                ...
                constant RegularTextures & texArgs  [[buffer(WKSVertTextureArgBuffer)]])
{
    ...
    outVert.position = basicPos(vert, uniforms, vertArgs.uniDrawState);
